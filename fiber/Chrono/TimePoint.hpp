...
namespace fiber{
    ...
    public:
        ...
        explicit constexpr TimePoint(std::chrono::duration<Rep, Period> duration) 
            : _duration(fiber::rounding_duration_cast<fiber::Duration>(duration)){}

        constexpr TimePoint& operator+=(Duration d){this->_duration += d; return *this;}
        constexpr TimePoint& operator-=(Duration d){this->_duration -= d; return *this;}
        
        template<class Rep, CRatio Period>
        constexpr TimePoint& operator+=(std::chrono::duration<Rep, Period> d){this->_duration += fiber::rounding_duration_cast<Duration>(d); return *this;}
        
        template<class Rep, CRatio Period>
        constexpr TimePoint& operator-=(Duration d){this->_duration -= fiber::rounding_duration_cast<Duration>(d); return *this;}
        ...
        friend constexpr TimePoint operator+ (TimePoint lhs, std::chrono::duration<Rep, Period> rhs){
            return lhs + fiber::rounding_duration_cast<Duration>(rhs);
        }
        ...
        friend constexpr TimePoint operator+ (std::chrono::duration<Rep, Period> lhs, TimePoint rhs){
            return fiber::rounding_duration_cast<Duration>(lhs) + rhs;
        }
