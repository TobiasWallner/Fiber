...
#include <fiber/Core/type_traits.hpp>
#include <fiber/Chrono/rounding_duration_cast.hpp>
#include <fiber/math/math.hpp>
#include <fiber/Exceptions/Exceptions.hpp>

namespace fiber{

    #ifdef FIBER_FMT_MINIMAL
        #if defined(FIBER_FMT_FLOAT_DECIMALS_1)
            #define FIBER_FMT_DECIMALS 1
        #elif defined(FIBER_FMT_FLOAT_DECIMALS_2)
            #define FIBER_FMT_DECIMALS 2
        #elif defined(FIBER_FMT_FLOAT_DECIMALS_3)
            #define FIBER_FMT_DECIMALS 3
        #elif defined(FIBER_FMT_FLOAT_DECIMALS_4)
            #define FIBER_FMT_DECIMALS 4
        #elif defined(FIBER_FMT_FLOAT_DECIMALS_5)
            #define FIBER_FMT_DECIMALS 5
        #elif defined(FIBER_FMT_FLOAT_DECIMALS_6)
            #define FIBER_FMT_DECIMALS 6
        #endif

        #if defined(FIBER_FMT_FLOAT_REP_SCI)
            #define FIBER_FMT_FLOAT_REP FloatRepresentation::Scientific
        #elif defined(FIBER_FMT_FLOAT_REP_ENG)
            #define FIBER_FMT_FLOAT_REP FloatRepresentation::Engineering
        #else // defined(FIBER_FMT_FLOAT_REP_FULL)
            #define FIBER_FMT_FLOAT_REP FloatRepresentation::Full
        #endif

        #ifdef FIBER_FMT_DOT_AS_COMMA
            #define FIBER_FMT_COMMA_CHAR '.'
            #define FIBER_FMT_THOUSANDS_CHAR ','
        #else
            #define FIBER_FMT_COMMA_CHAR ','
            #define FIBER_FMT_THOUSANDS_CHAR '.'
        #endif

        #ifdef FIBER_FMT_BOOL_TO_TEXT
            #define FIBER_FMT_BOOL_TO_TEXT_VALUE true
        #else 
            #define FIBER_FMT_BOOL_TO_TEXT_VALUE false
        #endif

        #ifdef FIBER_FMT_THOUSANDS
            #define FIBER_FMT_THOUSANDS_VALUE true
        #else 
            #define FIBER_FMT_THOUSANDS_VALUE false
        #endif

        #ifdef FIBER_FMT_PAD_SIGN
            #define FIBER_FMT_PAD_SIGN_VALUE true
        #else 
            #define FIBER_FMT_PAD_SIGN_VALUE false
        #endif

        #ifdef FIBER_FMT_FORCE_COMMA
            #define FIBER_FMT_FORCE_COMMA_VALUE true
        #else 
            #define FIBER_FMT_FORCE_COMMA_VALUE false
        #endif

        #ifdef FIBER_FMT_FORCE_DECIMALS
            #define FIBER_FMT_FORCE_DECIMALS_VALUE true
        #else
            #define FIBER_FMT_FORCE_DECIMALS_VALUE false
        #endif

        #ifdef FIBER_FMT_FORCE_SIGN
            #define FIBER_FMT_FORCE_SIGN_VALUE true
        #else
            #define FIBER_FMT_FORCE_SIGN_VALUE false
        #endif

        #ifdef FIBER_FMT_FORCE_EXPONENT
            #define FIBER_FMT_FORCE_EXPONENT_VALUE true
        #else
            #define FIBER_FMT_FORCE_EXPONENT_VALUE false
        #endif

        #ifdef FIBER_FMT_FORCE_EXPONENT_SIGN
            #define FIBER_FMT_FORCE_EXPONENT_SIGN_VALUE true
        #else
            #define FIBER_FMT_FORCE_EXPONENT_SIGN_VALUE false
        #endif
    #endif
    ...
    struct FormatBoolParam : public FormatStrParams{
        #ifndef FIBER_FMT_MINIMAL
            bool _to_text = true;
        #else
            static constexpr bool _to_text = FIBER_FMT_BOOL_TO_TEXT_VALUE;
        #endif
...
// -----------------------------------------------------------------------------------------------
    ...
    struct _FormatUIntParams{
        ...
        #else
            static constexpr char _thousands_char = FIBER_FMT_THOUSANDS_CHAR;
            static constexpr bool _use_thousands = FIBER_FMT_THOUSANDS_VALUE;
        #endif
    };
    ...
    struct FormatIntParams : public FormatStrParams, public _FormatUIntParams{
        ...
        #else
            static constexpr bool _force_sign = FIBER_FMT_FORCE_SIGN_VALUE;
            static constexpr bool _pad_sign = FIBER_FMT_PAD_SIGN_VALUE;
        #endif
    ...
    std::string_view uint_to_string(char* buffer_first, char* buffer_last, UInt value, bool use_thousands_char = false, char thousands_char = ','){
        if constexpr (!std::same_as<UInt, typename fiber::make_fast<UInt>::type>){
            return uint_to_string(buffer_first, buffer_last, static_cast<typename fiber::make_fast<UInt>::type>(value), use_thousands_char, thousands_char);
        }else{
    ...
    inline void print(OStream& stream, const Int& value, const FormatIntParams& params){
        using FInt = typename fiber::make_fast<Int>::type;
...
// -----------------------------------------------------------------------------------------------
    ...
    inline void print(OStream& stream, const Int& value, std::string_view suffix, const FormatIntSuffixParams& params){
        using FastInt = fiber::make_fast<Int>::type;
...
// -----------------------------------------------------------------------------------------------
    ...
    struct FormatFloatParams : public FormatIntParams{
        ...
        #else
            static constexpr FloatRepresentation _representation = FIBER_FMT_FLOAT_REP;
            static constexpr char _comma = FIBER_FMT_COMMA_CHAR;
            static constexpr unsigned int _decimals = FIBER_FMT_DECIMALS;
            static constexpr bool _force_comma = FIBER_FMT_FORCE_COMMA_VALUE;
            static constexpr bool _force_decimals = FIBER_FMT_FORCE_DECIMALS_VALUE;
            static constexpr bool _force_exponent = FIBER_FMT_FORCE_EXPONENT_VALUE;
            static constexpr bool _force_exponent_sign = FIBER_FMT_FORCE_EXPONENT_SIGN_VALUE;
        #endif
    ...
    struct str_add_float_params{
        ...
        #else
            static constexpr unsigned int decimals = FIBER_FMT_DECIMALS;
            static constexpr char comma_char = FIBER_FMT_COMMA_CHAR;
            static constexpr char thousands_char = FIBER_FMT_THOUSANDS_CHAR;
            static constexpr bool use_thousands = FIBER_FMT_THOUSANDS_VALUE;
            static constexpr bool force_comma = FIBER_FMT_FORCE_COMMA_VALUE;
            static constexpr bool force_decimals = FIBER_FMT_FORCE_DECIMALS_VALUE;
            static constexpr bool force_sign = FIBER_FMT_FORCE_SIGN_VALUE;
            static constexpr bool force_exponent = FIBER_FMT_FORCE_EXPONENT_VALUE;
            static constexpr bool force_exponent_sign = FIBER_FMT_FORCE_EXPONENT_SIGN_VALUE;
        #endif
    };
...
// -----------------------------------------------------------------------------------------------
    ...
    auto format_chrono(std::chrono::duration<Rep, Period> duration){
        if constexpr (std::ratio_greater_equal<Period, typename std::chrono::years::period>::value){
            return FormatIntSuffix(fiber::rounding_duration_cast<std::chrono::years>(duration).count(), "Y");
        }else if constexpr (std::ratio_greater_equal<Period, typename std::chrono::months::period>::value){
            return FormatIntSuffix(fiber::rounding_duration_cast<std::chrono::months>(duration).count(), "M");
        }else if constexpr (std::ratio_greater_equal<Period, typename std::chrono::weeks::period>::value){
            return FormatIntSuffix(fiber::rounding_duration_cast<std::chrono::weeks>(duration).count(), "W");
        }else if constexpr (std::ratio_greater_equal<Period, typename std::chrono::days::period>::value){
            return FormatIntSuffix(fiber::rounding_duration_cast<std::chrono::day>(duration).count(), "D");
        }else if constexpr (std::ratio_greater_equal<Period, typename std::chrono::hours::period>::value){
            return FormatIntSuffix(fiber::rounding_duration_cast<std::chrono::hours>(duration).count(), "h");
        }else if constexpr (std::ratio_greater_equal<Period, typename std::chrono::minutes::period>::value){
            return FormatIntSuffix(fiber::rounding_duration_cast<std::chrono::minutes>(duration).count(), "m");
        }else if constexpr (std::ratio_greater_equal<Period, typename std::chrono::seconds::period>::value){
            return FormatIntSuffix(fiber::rounding_duration_cast<std::chrono::seconds>(duration).count(), "s");
        }else if constexpr (std::ratio_greater_equal<Period, typename std::chrono::milliseconds::period>::value){
            return FormatIntSuffix(fiber::rounding_duration_cast<std::chrono::milliseconds>(duration).count(), "ms");
        }else if constexpr (std::ratio_greater_equal<Period, typename std::chrono::microseconds::period>::value){
            return FormatIntSuffix(fiber::rounding_duration_cast<std::chrono::microseconds>(duration).count(), "us");
        }else /* if constexpr (std::ratio_greater_equal<Period, typename std::chrono::nanoseconds::period>::value) */ {
            return FormatIntSuffix(fiber::rounding_duration_cast<std::chrono::nanoseconds>(duration).count(), "ns");
        }
    }
    ...
    inline OStream& operator<<(OStreamRef stream, const T& value){
        FIBER_ASSERT_O1_MSG(stream.is_open(), "No output stream! S: Assign an output stream to `fiber::cout`, `fiber::cerr` or `fiber::clog`.");
