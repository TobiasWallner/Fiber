...
namespace std{
    ...
    /// @brief Overload `std::is_integral` with `std::true_type` for `embed::Tick`
    ...
    struct is_integral<embed::Tick<Int, MaxTick>> : std::true_type{};

    /// @brief Overload `std::is_unsigned` with `std::true_type` for `embed::Tick`
    /// @tparam UInt an unsigned integer/integral
    /// @tparam MaxTick the maximum number of representable ticks before overflow
    template<std::unsigned_integral UInt, UInt MaxTick>
    struct is_unsigned<embed::Tick<UInt, MaxTick>> : std::true_type{};
}